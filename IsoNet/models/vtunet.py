...
class PatchExpand_old(nn.Module):
...
class PatchExpand(nn.Module):
    def __init__(self, dim, dim_scale=4, channel_reduce = True, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.dim_scale = dim_scale
        self.channel_reduce = channel_reduce
        if channel_reduce:
            self.expand = nn.Linear(dim, (dim_scale**3//2) * dim, bias=False)
    ...
    def forward(self, x):
        ...
        x: B, H*W, C
        """
        #print("expandx4_1",x.size())
        D, H, W = x.size()[2], x.size()[3],x.size()[4]#self.input_resolution
        #x = x.permute(0, 4, 1, 2, 3)
        #print("expandx4_2",x.size())
        x = x.flatten(2).transpose(1, 2)
        #print("expandx4_3",x.size())
        x = self.expand(x)
        #print("expandx4_4",x.size())
        B, L, C = x.shape
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, patch_size=(4, 4, 4), in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # print("patch_embed_0",patch_size)
        #self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        self.proj = nn.Sequential(
            nn.Conv3d(in_chans, embed_dim, kernel_size=(3,3,3), padding =1),
            nn.LeakyReLU(),
            nn.Conv3d(embed_dim, embed_dim, kernel_size=(2,2,2), stride=patch_size)
        )
    ...
    def forward(self, x):
        ...
        # print("patch_embed_2",x.size())
        x = self.proj(x)  # B C D Wh Ww
        # print("patch_embed_3",x.size())
        if self.norm is not None:
            D, Wh, Ww = x.size(2), x.size(3), x.size(4)
            x = x.flatten(2).transpose(1, 2)
            # print("patch_embed_4",x.size())
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.embed_dim, D, Wh, Ww)
        # print("patch_embed_5",x.size())
...
class VTUnet(nn.Module):
    ...
    def __init__(self, pretrained=None,
        ...
        if self.final_upsample == "expand_first":
            ...
            #self.up = PatchExpand(input_resolution=(
            #    img_size[0] // patch_size[0], img_size[1] // patch_size[1], img_size[2] // patch_size[2]),
            #    dim_scale=2,channel_reduce = False, dim=embed_dim)
            self.up=nn.ConvTranspose3d(in_channels=embed_dim, stride=(2,2,2), out_channels=embed_dim, kernel_size=4, padding=1)
            self.up_activation = nn.LeakyReLU()
    ...
    def up_x2(self, x):
        ...
        if self.final_upsample == "expand_first":
            #print("up_x4_2",x.size())
            x = self.up(x)
            x = self.up_activation(x)
            #print("up_x4_3",x.size())
